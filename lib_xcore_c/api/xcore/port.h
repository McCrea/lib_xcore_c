 ...
 *
 * Port resources must be allocated by name rather than from a pool.
 */
typedef enum {
  port_1A  = XS1_PORT_1A,
  port_1B  = XS1_PORT_1B,
 ...
 *  variable of type \c port before use. port_free() must be called afterwards.
 *
 ...
 *  \exception  ET_RESOURCE_DEP       another core is actively changing the port.
 */
inline void port_free(port_id_t p)
{
  _port_free(p);
}
