...
#define _INTERRUPT_PERMITTED(root_function) \
    __xcore_interrupt_permitted_ ## root_function
...
#define _DEFINE_INTERRUPT_PERMITTED_DEF(grp, root_function) \
    .weak  __xcore_fptrgroup.grp.nstackwords.group; \
    .max_reduce __xcore_fptrgroup.grp.nstackwords, __xcore_fptrgroup.grp.nstackwords.group, 0; \
    .set __xcore_kstack_words, _XCORE_STACK_ALIGN(_fptrgroup.grp.nstackwords $M XCORE_C_KSTACK_WORDS); \
    ...
    _INTERRUPT_PERMITTED(root_function):; \
      ...
      stw r5, sp[2]; \
      stw r4, sp[1]; \
      ldc r4, __xcore_kstack_words; \
      ldap r11, root_function; \
      add r5, r11, 0; \
      ldap r11, __xcore_interrupt_permitted_common; \
      bau r11; \
    .cc_bottom _INTERRUPT_PERMITTED(root_function).function; \
    .set   _INTERRUPT_PERMITTED(root_function).nstackwords, _XCORE_STACK_ALIGN(3) + __xcore_kstack_words + __xcore_interrupt_permitted_common.nstackwords + root_function.nstackwords; \
    .globl _INTERRUPT_PERMITTED(root_function).nstackwords; \
    .set   _INTERRUPT_PERMITTED(root_function).maxcores, 1 $M __xcore_interrupt_permitted_common.maxcores $M root_function.maxcores; \
    .globl _INTERRUPT_PERMITTED(root_function).maxcores; \
    .set   _INTERRUPT_PERMITTED(root_function).maxtimers, 0 $M __xcore_interrupt_permitted_common.maxtimers $M root_function.maxtimers; \
    .globl _INTERRUPT_PERMITTED(root_function).maxtimers; \
    .set   _INTERRUPT_PERMITTED(root_function).maxchanends, 0 $M __xcore_select_callback_common.maxchanends $M root_function.maxchanends; \
...
#define _DEFINE_INTERRUPT_PERMITTED(grp, ret, root_function, ...) \
    asm(_XCORE_STR(_DEFINE_INTERRUPT_PERMITTED_DEF(grp, root_function))); \
...
#define _DEFINE_INTERRUPT_CALLBACK_DEF(grp, intrpt) \
    ...
    .weak __xcore_fptrgroup.grp.nstackwords.group; \
    .add_to_set __xcore_fptrgroup.grp.nstackwords.group, _INTERRUPT_CALLBACK(intrpt).nstackwords, _INTERRUPT_CALLBACK(intrpt); \
    ...
    .set   _INTERRUPT_CALLBACK(intrpt).nstackwords, _XCORE_STACK_ALIGN(7) + __xcore_interrupt_callback_common.nstackwords + intrpt.nstackwords; \
    .globl _INTERRUPT_CALLBACK(intrpt).nstackwords; \
    .set   _INTERRUPT_CALLBACK(intrpt).maxcores, 1 $M __xcore_interrupt_callback_common.maxcores $M intrpt.maxcores; \
    .globl _INTERRUPT_CALLBACK(intrpt).maxcores; \
    .set   _INTERRUPT_CALLBACK(intrpt).maxtimers, 0 $M __xcore_interrupt_callback_common.maxtimers $M intrpt.maxtimers; \
    .globl _INTERRUPT_CALLBACK(intrpt).maxtimers; \
    .set   _INTERRUPT_CALLBACK(intrpt).maxchanends, 0 $M __xcore_interrupt_callback_common.maxchanends $M intrpt.maxchanends; \
...
#define _DEFINE_INTERRUPT_CALLBACK(grp, intrpt, data) \
    asm(_XCORE_STR(_DEFINE_INTERRUPT_CALLBACK_DEF(grp, intrpt))); \
