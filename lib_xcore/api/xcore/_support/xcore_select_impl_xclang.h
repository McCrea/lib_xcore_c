...
#define _XCORE_SELECT_WAIT_HANDLER__XCORE_GTYPE_NONE(COND_, LABEL, ...) \
  _XCORE_SELECT_TAKE_EVENT_NONBLOCKING(__VA_ARGS__); \
  goto LABEL;


#define _XCORE_SELECT_WAIT_HANDLER__XCORE_NO_DEFAULT_CASE(COND_, LABEL_, ...) \
  asm goto("waiteu" \
...
#define _XCORE_SELECT_WAIT_HANDLER__XCORE_GTYPE_TRUE(COND, LABEL, ...) \
  do { \
    const int __xcore_wait_cond = COND; \
    if (__builtin_constant_p(__xcore_wait_cond)) { \
      if (__xcore_wait_cond) { _XCORE_SELECT_TAKE_EVENT_NONBLOCKING(__VA_ARGS__); goto LABEL; } \
      else { _XCORE_SELECT_WAIT_HANDLER__XCORE_NO_DEFAULT_CASE(,, __VA_ARGS__) } \
    } else { \
      _XCORE_SELECT_TAKE_EVENT_NONBLOCKING(__VA_ARGS__); \
      asm goto("waitef %[cond]" \
        : /* Can't have outputs */ \
        : [cond] "r"(__xcore_wait_cond) \
        : /* No clobbers */ \
        : __VA_ARGS__); \
...
#define _XCORE_SELECT_WAIT_HANDLER__XCORE_GTYPE_FALSE(COND, LABEL, ...) \
  do { \
    const int __xcore_wait_cond = COND; \
    if (__builtin_constant_p(__xcore_wait_cond)) { \
      if (__xcore_wait_cond) { _XCORE_SELECT_WAIT_HANDLER__XCORE_NO_DEFAULT_CASE(,, __VA_ARGS__); } \
      else { _XCORE_SELECT_TAKE_EVENT_NONBLOCKING(__VA_ARGS__); goto LABEL; } \
    } else { \
      _XCORE_SELECT_TAKE_EVENT_NONBLOCKING(__VA_ARGS__); \
...
#define _XCORE_SELECT_WAIT_HANDLER_I(LABEL, TYPE, COND, ...) _XCORE_SELECT_WAIT_HANDLER_NAME(TYPE)(COND, LABEL, __VA_ARGS__)
#define _XCORE_SELECT_WAIT_HANDLER(DEFAULT_PACK, ...) _XCORE_PSHIM(_XCORE_SELECT_WAIT_HANDLER_I, DEFAULT_PACK, __VA_ARGS__)

#define _XCORE_SELECT_RES_I(LNAME, DEFAULT_PACK, ...) \
  switch (0) for (const void *__xcore_select_reset;;) if (1) \
  { \
    _XCORE_APPLY_NOSEP(_XCORE_SELECT_RES_ENABLER_REPEAT, __VA_ARGS__) \
    _XCORE_SELECT_WAIT_HANDLER(DEFAULT_PACK, _XCORE_APPLY(_XCORE_LABEL, __VA_ARGS__)) \
  } \
  else if (0) \
  { \
  case 0: \
...
#define _XCORE_SELECT_RES_ENABLER_ORDERED_III(RES, LABEL_, GTYPE, EXPR, ...) _XCORE_SELECT_RES_ENABLER_ORDERED_NAME(GTYPE)(RES, EXPR, __VA_ARGS__)
...
#define _XCORE_SELECT_RES_ORDERED_II(LNAME, DEFAULT_PACK, LABELS, ...) \
  switch (0) for (const void *__xcore_select_reset;;) if (1) \
  { \
    __xcore_select_disable_trigger_all(); \
...
#define _XCORE_SELECT_RES_ORDERED_I(LNAME, DEFAULT_PACK, ...) _XCORE_SELECT_RES_ORDERED_II(LNAME, DEFAULT_PACK, (_XCORE_APPLY(_XCORE_LABEL, __VA_ARGS__)), __VA_ARGS__)
